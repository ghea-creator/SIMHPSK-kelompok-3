...
class SaleController extends Controller
{
    ...
    public function storeApi(Request $request)
    {
        ...
        $currentStock = \App\Models\StockTransaction::getCurrentBalance($request->user()->id);
        if ($weightKg > $currentStock) {
            return $this->errorResponse("Stok gudang tidak mencukupi. Sisa stok: {$currentStock} kg.", 422);
        }
    ...
    public function updateApi(Request $request, Sale $sale)
    {
        ...
        if (array_key_exists('buyer_address', $validated)) $dbData['buyer_address'] = $validated['buyer_address'];
        ...
        if ($weightKg !== null) {
            if ($weightKg > $sale->weight_kg) {
                $difference = $weightKg - $sale->weight_kg;
                $currentStock = \App\Models\StockTransaction::getCurrentBalance($request->user()->id);
                if ($difference > $currentStock) {
                    return $this->errorResponse("Stok gudang tidak mencukupi untuk penambahan ini. Sisa stok: {$currentStock} kg.", 422);
                }
            }
            $dbData['weight_kg'] = $weightKg;
        }
