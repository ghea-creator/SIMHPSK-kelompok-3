...
class HarvestController extends Controller
{
    ...
    public function destroy(Request $request, Harvest $harvest)
    {
        ...
        // Web response - also rollback stock
        $harvest->delete();
    ...
    public function destroyApi(Request $request, Harvest $harvest)
    {
        ...
        // Rollback stock: kurangi stok sebesar berat panen yang dihapus
        $currentBalance = StockTransaction::getCurrentBalance($request->user()->id);
        if ($harvest->weight_kg > 0 && $currentBalance > 0) {
            $rollback = min($harvest->weight_kg, $currentBalance); // tidak boleh minus
            StockTransaction::addTransaction(
                'out',
                $rollback,
